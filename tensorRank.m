function [xtrue, xhist, flag, sumx, diffsumx, rho, flagr] ...
    = tensorRank(alpha, R, v, x, gamma)
% [xtrue, xhist, flag, sumx, diffsumx, rho, flagr] = tensorRank(alpha, R, v, x, gamma)
...
% rho records the ratio of ||x^(k+1) - x^(k)||_1 / ||x^(k) - x^(k-1)||_1
...
for n = 1 : niter
    ...
    xhist(:, n) = xn;
    if n > 2
            rho(n - 2) = norm(xhist(:,n) - xhist(:,n-1), 1) / norm(xhist(:,n-1) - xhist(:,n-2), 1);
			if rho(n - 2) > 1
				flagr = 1;
			end
    end
